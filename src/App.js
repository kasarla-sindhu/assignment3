...
class App extends Component {
  ...
  imgSelected = Id => {
    const {imgurl, timeInSec} = this.state
    if (imgurl.id === Id) {
      if (timeInSec !== 0) {
        this.setState(prevState => ({
          count: prevState.count + 1,
          imgurl: imagesList[Math.floor(Math.random() * imagesList.length)],
        }))
      } else {
        clearInterval(this.timerId)
  ...
  render() {
    const {count, timeInSec, imgurl, activeTab, iscardShown} = this.state
    ...
    if (iscardShown) {
      Element = (
        <div className="bottom-con1">
          <div className="score-bg">
            ...
            <button type="button" className="butn" onClick={this.playagain}>
              <img
      ...
      Element = (
        <div className="bottom-con">
          <div className="random-img-con">
            <img className="img" src={imgurl.imageUrl} alt="match" />
          </div>
          <ul className="btns-con">
            {tabsList.map(each => (
              <Buttons
                details={each}
                key={each.tabId}
    ...
    return (
      <div className="container">
        <Navbar count={count} timeInSec={timeInSec} />
        {Element}
      </div>
    )
  }
